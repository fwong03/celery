...
- The ``backend`` argument specifies the result backend to use.
    ...
    The result backend is used to keep track of task states and results.
    While results are disabled by default, I use the RPC result backend here
    because I will demonstrate how to retrieve results work later. You may want
    to use a different backend for your application. They all have different
...
-- *Concurrency* is the number of prefork worker processes used
to process your tasks concurrently. When all of these are busy doing work,
a new task will have to wait for one of the tasks to finish before
it can be processed.
...
These options are described in more detail in the :ref:`Workers Guide <guide-workers>`.
...
The daemonization scripts use the :program:`celery multi` command to
...
With the multi command you can start multiple workers. There is powerful
...
to use. It must be in the form ``module.path:attribute``

But it also supports a shortcut form: if only a package name is specified,
it will try to search for the app instance in the following order:
...
If none of these are found it'll then search a submodule named ``proj.celery``:
...
(``__call__``), represent the Celery calling API and are also used for
signatures.
...
instance that can be used to keep track of a task's execution state.
But for this you need to enable a :ref:`result backend <task-result-backends>` so that
the state can be stored somewhere.

Because there is no result backend that suits every application, results are
disabled by default. To choose one, you need to consider the drawbacks of each
individual backend. For many tasks keeping the return value isn't even very
useful, so no result backend is a sensible default. Also note that result
backends aren't used for monitoring tasks and workers; for that, Celery uses
dedicated event messages (see :ref:`guide-monitoring`).
...
While a task can only be in a single state, it can progress through several
states. The stages of a typical task could be::
...
the default state for any task id that's unknown. You can see
...
You just learned how to call a task using the task's ``delay`` method.
...
invocation in a way that it can be passed to functions or even serialized
